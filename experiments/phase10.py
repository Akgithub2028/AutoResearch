# %% [markdown]
# # Phase 10: Long-Term Memory (Reflexion)
# This notebook implements Phase 10 of the AutoResearch project.
# Goal: Compress the execution history into semantic and procedural memory upon task completion.
...
class ResearchState(TypedDict, total=False):
    ...
    memory: Dict[str, Any] # Populated in this Phase
    citations: List[str]
...
def memory_node(state: ResearchState) -> dict:
    """
    NEW: Memory Node. Distills semantic and procedural knowledge from the run.
    """
    query = state.get("query", "")
    draft = state.get("draft", "")
    history = state.get("execution_history", [])
    
    print(f"[Memory Node] Summarizing {len(history)} execution steps into memory.")
...
def route_after_reflection(state: ResearchState) -> str:
    ...
    if is_satisfactory:
        print(f"[Router] -> Reflection Satisfactory. Routing to 'memory'.")
        return "memory"
    elif rev_count >= 2:
        print(f"[Router] -> Max revisions ({rev_count}) reached. Routing to 'memory'.")
        return "memory"
    else:
...
def build_graph():
    ...
    builder.add_node("memory", memory_node) # NEW
    ...
    # Conditional edge after reflection now routes to memory upon completion
...
def run_phase10_validation():
    ...
    graph = build_graph()
    config = {"configurable": {"thread_id": "phase10_test_thread"}}
    ...
    print(f"\n[Generated Long-Term Memory Preview]")
    memory = final_state.get('memory', {})
    print(f"Semantic Memory: {memory.get('semantic_memory')}")
    print(f"Procedural Memory: {memory.get('procedural_memory')}")
    print(f"Episodic Memory Steps Logged: {len(memory.get('episodic_memory', []))}")
